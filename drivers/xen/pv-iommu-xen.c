...
/* For Xen foreign pages, map a single buffer of the indicated size for
 * DMA in streaming mode.  The physical address to use is returned.
 *
 * The original MFN offset into previously setup IOMMU 1-to-1 offset mapping
 * of host memory is used.
 *
 * For non-foreign pages, function returns false so that the caller can
 * map the page with the normal x86 map functions.
...
{
	struct io_tlb_mem *p_io_tlb_mem = get_io_tlb_default_mem_addr();
	...
	if (p2m_entry & FOREIGN_FRAME_BIT) {
		...
		if (dma_capable(dev, phys, size, true)) {
                        *phys_addr = phys;
		} else {
			phys_addr_t map = swiotlb_tbl_map_single(
						dev, page_to_phys(page) + offset,
						size, size, 0, dir, attrs);
			trace_swiotlb_bounced(dev, phys, size);
			*phys_addr = phys_to_dma(dev, map);
		}
		return true;
	}
	return false;
			 ...
			 unsigned long attrs)
{
	...
	for_each_sg(sgl, sg, nelems, i) {
		...
		if (!dma_capable(hwdev, dev_addr, sg->length, true)){
			phys_addr_t map = swiotlb_tbl_map_single(hwdev,
						paddr, sg->length, sg->length, 0, dir, attrs);
			trace_swiotlb_bounced(hwdev, dev_addr, sg->length);
			if (map == (phys_addr_t)DMA_MAPPING_ERROR) {
				dma_unmap_sg_attrs(hwdev, sgl, i, dir, attrs);
				sgl[0].dma_length = 0;
				return 0;
			}
