...
static int xen_rebind_evtchn_to_cpu_kabi(struct irq_info *info, unsigned int tcpu)
{
...
{
	struct irq_info *info = info_for_irq(irq_from_evtchn(evtchn));

	if (!info)
		return -1;

	return xen_rebind_evtchn_to_cpu_kabi(info, tcpu);
}
			    ...
			    bool force)
{
	...
	int ret = xen_rebind_evtchn_to_cpu_kabi(info_for_irq(data->irq), tcpu);
