...
static int gntpool_chunk_alloc(struct grant_pool_chunk *chunk, size_t order)
{
  ...
  printk("chunk->pages = %lx", (unsigned long)chunk->pages);
  if (!chunk->pages) {
    rc = -ENOMEM;
    goto out;
  }
...
int xen_gntpool_create(struct grant_pool *pool, size_t order)
{
  ...
  pool->nr_chunks = 1 << count_order;

  pool->chunks = kvcalloc(pool->nr_chunks, sizeof(struct grant_pool_chunk), GFP_KERNEL);
  if (!pool->chunks)
    return -ENOMEM;

  for (size_t i = 0; i < pool->nr_chunks; i++) {
    WARN_ON(gntpool_chunk_alloc(&pool->chunks[i], size_order));
    printk("chunk->chunks[%zu] = bitmap=%lx first=%u nr_pages=%zx\n", i,
	  (unsigned long)pool->chunks[i].bitmap, pool->chunks[i].first,
	  pool->chunks[i].nr_pages);
  }
