...
struct napi_struct {
	...
	UEK_KABI_USE(3, int napi_rmap_idx)
	UEK_KABI_RESERVE(4)
	UEK_KABI_EXTEND_WITH_SIZE(struct irq_affinity_notify notify, 10)
};
...
struct net_device {
	...
	UEK_KABI_FILL_HOLE(bool	irq_affinity_auto)
	UEK_KABI_FILL_HOLE(bool rx_cpu_rmap_auto)
#ifdef CONFIG_XPS
...
#define TSO_MAX_SEGS		U16_MAX
	...
	UEK_KABI_DEPRECATE(unsigned long,	netns_local:1)
	unsigned long		fcoe_mtu:1;
	UEK_KABI_FILL_HOLE(unsigned long	netns_immutable:1)
	 ...
	 *
	 * Protects:
	 *	@napi_list, @net_shaper_hierarchy, @reg_state, @threaded, @dev_addr
	 *
