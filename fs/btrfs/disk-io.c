...
static void btrfs_init_csum_hash(struct btrfs_fs_info *fs_info, u16 csum_type)
{
	...
	case BTRFS_CSUM_TYPE_CRC32:
		/*
		 * arm64 provides hardware CRC32 via arch/arm64/lib/crc32.S,
		 * compiled in unconditionally with CONFIG_CRC32; x86 one is
		 * in arch/x86/crypto/crc32c-intel_glue.c and requires
		 * CONFIG_CRYPTO_CRC32C_INTEL.
		 */
		if (IS_ENABLED(CONFIG_CRYPTO_CRC32C_INTEL) ||
